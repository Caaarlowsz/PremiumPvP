...
public class RemoveTokens implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length != 3) {
            sender.sendMessage(Text.color("&cPlease use /cosmotokens remove <player> <amount>"));
            return true;
        }
        try {
            ...
            try {
                ...
                sender.sendMessage(Text.color("&aYou have removed " + value + " tokens from " + player.getName() + "'s account."));
                return true;
            } catch (Exception ex) {
